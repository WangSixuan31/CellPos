...
from torch.nn import Sequential, Linear, BatchNorm1d, LayerNorm, Dropout, Sigmoid, ReLU, LeakyReLU
...
class Encoder(nn.Module):
    ...
    def forward(self, x, edge_index, spatial_coords=None):
        ...
        z = self.hidden_layer3(hidden_out2, edge_index)
        z = self.batch_norm3(z) 
        z = nn.LeakyReLU()(z)
        
        attn_spatial = None
        if spatial_coords is not None and self.spatial_embedding is not None:
            spatial_embedded = self.spatial_embedding(spatial_coords)  
...
class GraphAutoEncoder(nn.Module):
    ...
    def forward(self, x, edge_index, spatial_coords=None):
        if spatial_coords is not None:
            z = self.encoder(x, edge_index, spatial_coords)
        else:
...
class MultiTaskModel(nn.Module):
    def __init__(self, input_dim, hidden_dims,dropout,
        ...
        self.fc_position = nn.Linear(hidden_dims[2], output_coord_dim)  # Predicting positions
...
def get_weight_dist_loss(model1, model2):
    dist = 0
