...
def preprocess(sc_adata, st_adata,  
    ...
    sc.pp.filter_cells(sc_adata, min_genes=sc_min_genes) 
...
def knn_graph(x, k=30, metric: str = 'euclidean',):
    """
    :param x: gene expression (scRNA-seq data) or spatial coordinates (ST data)
...
def gaussian_kernel_adjacency_matrix(x, metric: str = 'cosine', top_k = 20):
    ...
    sigma = np.mean(np.sqrt(np.maximum(mean_sq - mean ** 2, 0)))
...
def get_data_loader(adata, batch_size: int = 32, data_type: str = 'scRNA-seq', seed: int = 2026):
    """
    data_type: 'scRNA-seq' or 'spatial'
    """
...
def sort_edge_index(edge_index):
    ...
    return torch.tensor(edge_index[:, idx], dtype=torch.long)
    
def get_graph_data_loader(adata, edge_index, batch_size: int = 32, seed: int = 2026):
    ...
    return DataLoader(dataset = [graph_data], batch_size = batch_size, shuffle=False, num_workers=0,
                      worker_init_fn=seed_worker, generator=g)

def construct_graph_data(sc_adata, st_adata, metric: str = 'cosine', top_k: int = 20, 
                         batch_size: int = 32, seed: int = 2026):
    ...
    sc_dataloader = get_graph_data_loader(sc_adata, sc_edge_index, batch_size=batch_size, seed=seed)
    st_dataloader = get_graph_data_loader(st_adata, st_edge_index, batch_size=batch_size, seed=seed)
